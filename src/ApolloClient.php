...
class ApolloClient
{
    ...
    protected $isServer = false; //是否是服务
    ...
    public function setSaveDir($configDir)
    {
        if (!file_exists($configDir)) {
            $flag = mkdir($configDir, 0777, true);

            if(!$flag) return false;
        }
        $this->saveDir = $configDir;
    }

    /**
     * 设置版本（脚本需要设置）
     * $notifyArr = array($namespace => $notificationId)
     */
    public function serNotifications($notifyArr = array())
    {
        if ($notifyArr) {
            foreach ($this->notifications as $namespace => $notifications) {
                if (isset($notifyArr[$namespace])) {
                    $notifications['notificationId'] = $notifyArr[$namespace];
                }
                $this->notifications[$namespace] = $notifications;
            }
        }
    }

    public function setIsServer($bool = false)
    {
        $this->isServer = (bool)$bool;
    }
